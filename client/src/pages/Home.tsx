...
export default function Home() {
  ...
  return <div className="store-shell"><div className="site-background-overlay" aria-hidden="true" />
    ...
    <main>
      ...
      <section className="faq-section"><div className="section-heading"><div><p className="eyebrow accent">DÚVIDAS FREQUENTES</p><h2>Antes de decidir.</h2></div></div><Accordion type="single" collapsible className="faq-list">{["Os produtos são originais?", "Qual a garantia dos aparelhos novos?", "Qual a garantia dos seminovos?", "Os seminovos possuem caixa?", "Posso comprar pelo site?", "Como funciona o pagamento?"].map((question, i) => <AccordionItem value={`item-${i}`} key={question}><AccordionTrigger>{question}</AccordionTrigger><AccordionContent>{i === 2 ? "Os seminovos possuem garantia padrão de 90 dias da loja, conforme cadastro do aparelho." : i === 3 ? "Os seminovos são aparelhos sem caixa. Todos os detalhes são exibidos individualmente na vitrine." : "Nossa equipe confirma todos os detalhes de disponibilidade, pagamento e entrega diretamente pelo WhatsApp."}</AccordionContent></AccordionItem>)}</Accordion></section>
    </main>
    ...
    <Dialog open={Boolean(selectedAccessory)} onOpenChange={(open) => !open && setSelectedAccessory(null)}>
